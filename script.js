...
const list=[{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
//Mapping all data with table and showing it to UI
function showData(list,tableBody){
...
for(let i=0;i<list.length;i++){
    ...
    <td id="name-container">
                        <div id="details">
                            <img src="https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1" alt="">
                            Minhaj
                        </div>
                    </td> 
        </tr>*/
    ...
    let detailsContainer=document.createElement("div");
     tableBody.id==="table-body"?detailsContainer.id="details":detailsContainer.id="details2";
    ...
    cls.innerText=element.class
    marks.innerText=element.marks
    passing.innerText=element.passing==true?"Passed":"Failed"
...
showData(list,tableBody);

// --> searching elements
let input=document.getElementById("search")
let btn=document.getElementById("search-btn");
btn.addEventListener("click",searchElement);
    ...
    if(newList.length==0){
        for(let i=0;i<list.length;i++){
            ...
            let element=list[i];
                ...
                if(name.innerText.toLowerCase().includes(searchString)||email.innerText.toLowerCase().includes(searchString)){
     ...
     showData(resList,tableBody);
     newList=resList;
}

// --> sort by Ascending name (A->Z)

let ascNameBtn=document.getElementById("asc-name");
ascNameBtn.addEventListener("click",sortAsc);
function sortAsc(){


    table2.style.display="none"
    if(newList.length>0){
        newList.sort(compareAscName);
        showData(newList,tableBody);
        }
        
        else{
            list.sort(compareAscName);
            showData(list,tableBody);
     }
}
function compareAscName(o1,o2){
    let fullName_1=o1.first_name+" "+o1.last_name;
    let fullName_2=o2.first_name+" "+o2.last_name;

return fullName_1.localeCompare(fullName_2);
}

// --> sort by Descending Name (Z->A)
let descNameBtn=document.getElementById("des-name");
descNameBtn.addEventListener("click",sortDesc);
function sortDesc(){
 

    table2.style.display="none"
    if(newList.length>0){
    newList.sort(compareDescName);
    showData(newList,tableBody);
    }
    
    else{
        list.sort(compareDescName);
        showData(list,tableBody);
    }
}
function compareDescName(o1,o2){
    let fullName_1=o1.first_name+" "+o1.last_name;
    let fullName_2=o2.first_name+" "+o2.last_name;

return fullName_2.localeCompare(fullName_1);
...
// --> sort by Marks (In Ascending order)
  ...
  marksBtn.addEventListener("click",sortByMarks);
  function sortByMarks(){ 

    table2.style.display="none"
    if(newList.length>0){
    newList.sort(compareMarks);
    showData(newList,tableBody);
    }
    else{
        list.sort(compareMarks)
        showData(list,tableBody);
    }
    

  }
  function compareMarks(o1,o2){
  ...
  passingBtn.addEventListener("click",sortByPassing);
  function sortByPassing() {
    
    table2.style.display="none"

    if(newList.length==0){
        let printList=list.filter((el)=>{
            if(el.passing===true) return el;
        })
        showData(printList,tableBody);
        newList=printList;
    }

    else{
        let printList=newList.filter((el)=>{
            if(el.passing===true) return el;
        })
        showData(printList,tableBody);
        newList=printList;
  ...
  classBtn.addEventListener("click",sortByClass);
  ...
  let sortByGenderBtn=document.getElementById("gender");
  sortByGenderBtn.addEventListener("click",sortByGender);
  function sortByGender(){
    table2.style.display="table"
    if(newList.length==0){
        let femaleList=list.filter((el)=>{
            if(el.gender==="Female"){
                return el;
            }
         })
          showData(femaleList,tableBody);
    
     let maleList=list.filter((el)=>{
        if(el.gender==="Male"){
            return el;
        }
     })
      showData(maleList,tableBody2);

    
    }
    else{

        let femaleList=newList.filter((el)=>{
            if(el.gender==="Female"){
                return el;
            }
         })
          showData(femaleList,tableBody);
        let maleList=newList.filter((el)=>{
           if(el.gender==="Male"){
               return el;
           }
        })
